...
algo_name = StringVar()
algo_list = ['Merge Sort' , 'Bubble Sort' , 'Insertion Sort' , 'Heap Sort' , 'Quick Sort' , 'Counting Sort' , 'Radix Sort' , 'Bucket Sort' , 'Optimized QuickSort']
...
def quick_sort(arr, start, end,
               displayArr, timeTick):
    if start < end:
        partitionIdx = partition(arr, start,
                                 end, displayArr,
                                 timeTick)
 
        # left partition
        quick_sort(arr, start, partitionIdx-1,
                   displayArr, timeTick)
 
        # right partition
        quick_sort(arr, partitionIdx+1,
                   end, displayArr, timeTick)
...
def sort():
    ...
    if algo_comboBox.get()=='Merge Sort':
        ...
        text_box.insert('end', message)
        text_box.config(state='disabled')
        # insertionSort(arr,0,len(arr)-1,displayArr,tym)
        # displayArr(arr, ['blue' for x in range(len(arr))])
        ...
        text_box.insert('end', message)
        text_box.config(state='disabled')
        optimizedQuickSort(arr,0,len(arr)-1 ,5, displayArr,tym)
        displayArr(arr , ['Blue' for x in range(len(arr))])    
